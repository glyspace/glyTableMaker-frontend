...
import { Typography, OutlinedInput, FormHelperText } from "@mui/material";
import CloseIcon from "@mui/icons-material/Close";
import { postJson, validateEmail } from "../utils/api";
...
const FeedbackWidget = (props) => {
  ...
  const handleSubmit = (e) => {
    ...
    postJson ("api/util/sendfeedback", formData).then ( (data) => {
      feedbackSuccess(data);
    }).catch (function(error) {
      if (error && error.response && error.response.data) {
          feedbackFailure(error.response.data);
      } else {
          props.setAlertDialogInput && axiosError(error, null, props.setAlertDialogInput);
      }
    });
  };
